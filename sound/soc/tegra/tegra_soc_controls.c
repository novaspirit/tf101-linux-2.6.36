...
extern struct headset_data *hs_data;
...
int tegra_play_route_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	//struct tegra_audio_data* audio_data = snd_kcontrol_chip(kcontrol);

	//compatibility fix for ALSA
	int play_device_new = (switch_get_state(&hs_data->sdev) ?
		TEGRA_AUDIO_DEVICE_OUT_HEADSET : TEGRA_AUDIO_DEVICE_OUT_SPEAKER);

	if (audio_data) {
		//int play_device_new = ucontrol->value.integer.value[0] & TEGRA_AUDIO_DEVICE_OUT_ALL;
...
int tegra_controls_init(struct snd_soc_codec *codec)
{
	...
	audio_data = codec->socdev->codec_data;
	int err;
